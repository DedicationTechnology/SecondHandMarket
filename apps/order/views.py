...
class OrderPayView(View):
    ...
    def post(self, request):
        ...
        # 初始化
        app_private_key_path = os.path.join('apps/order/app_private_key.pem')
        alipay_public_key_path = os.path.join('apps/order/alipay_public_key.pem')
        alipay = AliPay(
            appid="2021000117612981", # 应用id
            app_notify_url=None,  # 默认回调url
             # 支付宝的公钥，验证支付宝回传消息使用，不是你自己的公钥,
            app_private_key_string = open(app_private_key_path).read(),
            alipay_public_key_string = open(alipay_public_key_path).read(),
            sign_type="RSA2",  # RSA 或者 RSA2
            debug=True,  # 默认False，为Ture才会调用沙箱的地址
            config = AliPayConfig(timeout=15)
        )
        # 调用支付接口
        # 电脑网站支付，需要跳转到https://openapi.alipaydev.com/gateway.do? + order_string
        subject = '二手交易市场%s' % order_id
        total_pay = order.total_price+order.transit_price # Decimal
        order_string = alipay.api_alipay_trade_page_pay(
            out_trade_no=order_id, # 订单id
            total_amount=str(total_pay), # 支付总金额
            subject=subject,  # 标题
            return_url='https://www.baidu.com',  # 订单支付成功后回调的地址
